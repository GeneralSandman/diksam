...
static BuiltInMethodParameter st_array_insert_arg[] = {
    {"index", DVM_INT_TYPE}, {"new_item", DVM_BASE_TYPE}};
static BuiltInMethodParameter st_array_remove_arg[] = {{"index", DVM_INT_TYPE}};
...
static BuiltInMethodParameter st_string_substr_arg[] = {
    {"start", DVM_INT_TYPE}, {"length", DVM_INT_TYPE}};
...
static FunctionDefinition *create_built_in_method(BuiltInMethod *src,
                                                  int method_count) {
  int i;
  int param_idx;
  ...
  for (i = 0; i < method_count; i++) {
    ...
    for (param_idx = 0; param_idx < src[i].parameter_count; param_idx++) {
      ...
      if (param_list) {
        param_list = dkc_chain_parameter(param_list, type,
                                         src[i].parameter[param_idx].name);
      } else {
        param_list =
            dkc_create_parameter(type, src[i].parameter[param_idx].name);
      }
    }
...
DKC_Compiler *DKC_create_compiler(void) {
  ...
  compiler->array_method =
      create_built_in_method(st_array_method, ARRAY_SIZE(st_array_method));
  compiler->string_method_count = ARRAY_SIZE(st_string_method);
  compiler->string_method =
      create_built_in_method(st_string_method, ARRAY_SIZE(st_string_method));
...
static CompilerList *add_compiler_to_list(CompilerList *list,
                                          DKC_Compiler *compiler) {
  CompilerList *pos;
...
static DKC_Compiler *search_compiler(CompilerList *list,
...
static void make_search_path(int line_number, PackageName *package_name,
                             char *buf) {
  PackageName *pos;
  int len = 0;
  int prev_len = 0;
  ...
  for (pos = package_name; pos; pos = pos->next) {
    ...
    if (len > FILENAME_MAX - (2 + suffix_len)) {
      dkc_compile_error(line_number, PACKAGE_NAME_TOO_LONG_ERR,
...
static void get_require_input(RequireList *req, char *found_path,
  ...
  if (status != SEARCH_FILE_SUCCESS) {
    if (status == SEARCH_FILE_NOT_FOUND) {
      dkc_compile_error(req->line_number, REQUIRE_FILE_NOT_FOUND_ERR,
                        STRING_MESSAGE_ARGUMENT, "file", search_file,
                        MESSAGE_ARGUMENT_END);
    } else {
      dkc_compile_error(req->line_number, REQUIRE_FILE_ERR,
...
static DVM_Boolean add_exe_to_list(DVM_Executable *exe,
...
static void set_path_to_compiler(DKC_Compiler *compiler, char *path) {
  compiler->path =
      MEM_storage_malloc(compiler->compile_storage, strlen(path) + 1);
  strcpy(compiler->path, path);
}

static DVM_Executable *do_compile(DKC_Compiler *compiler,
                                  DVM_ExecutableList *list, char *path,
...
PackageName *create_one_package_name(DKC_Compiler *compiler, char *str,
                                     int start_idx, int to_idx) {
  PackageName *pn;
  int i;
...
SearchFileStatus get_dynamic_load_input(char *package_name, char *found_path,
                                        char *search_file,
                                        SourceInput *source_input) {
  SearchFileStatus status;
  char *search_path;
...
SearchFileStatus dkc_dynamic_compile(DKC_Compiler *compiler, char *package_name,
                                     DVM_ExecutableList *list,
  ...
  status = get_dynamic_load_input(package_name, found_path, search_file,
                                  &source_input);
  if (status != SEARCH_FILE_SUCCESS) {
    return status;
  }
...
static CompilerList *traversal_compiler(CompilerList *list,
                                        DKC_Compiler *compiler) {
