...
void DVM_add_native_function(DVM_VirtualMachine *dvm, char *func_name,
    ...
    dvm->function =
        MEM_realloc(dvm->function, sizeof(Function) * (dvm->function_count + 1));
        ...
        if (executable->function[src_idx].is_implemented) {
            ...
            for (dest_idx = 0; dest_idx < dvm->function_count; dest_idx++) {
                if (!strcmp(dvm->function[dest_idx].name,
                    ...
                    dvm_error(NULL, NULL, NO_LINE_NUMBER_PC, FUNCTION_MULTIPLE_DEFINE_ERR,
                              STRING_MESSAGE_ARGUMENT, "name",
                              dvm->function[dest_idx].name, MESSAGE_ARGUMENT_END);
                }
            }
        }
    }
    dvm->function = MEM_realloc(
        dvm->function, sizeof(Function) * (dvm->function_count + func_count));
...
static int search_function(DVM_VirtualMachine *dvm, char *name) {
    ...
    dvm_error(NULL, NULL, NO_LINE_NUMBER_PC, FUNCTION_NOT_FOUND_ERR,
              STRING_MESSAGE_ARGUMENT, "name", name, MESSAGE_ARGUMENT_END);
    return 0; /* make compiler happy */
}

static void convert_code(DVM_VirtualMachine *dvm, DVM_Executable *exe,
...
static void initialize_value(DVM_VirtualMachine *dvm, DVM_BasicType basic_type,
                             DVM_Value *value) {
    switch (basic_type) {
...
static void add_static_variables(DVM_VirtualMachine *dvm, DVM_Executable *exe) {
    ...
    for (i = 0; i < exe->global_variable_count; i++) {
        initialize_value(dvm, exe->global_variable[i].type->basic_type,
...
static DVM_Object *chain_string(DVM_VirtualMachine *dvm, DVM_Object *str1,
                                DVM_Object *str2) {
    int         len;
    DVM_Char *  str;
    DVM_Object *ret;
...
static void invoke_native_function(DVM_VirtualMachine *dvm, Function *func,
                                   int *sp_p) {
    DVM_Value *stack;
    int        sp;
    DVM_Value  ret;
    ...
    ret = func->u.native_f.proc(dvm, func->u.native_f.arg_count,
...
static void initialize_local_variables(DVM_VirtualMachine *dvm,
    ...
    for (i = 0, sp_idx = from_sp; i < func->local_variable_count; i++, sp_idx++) {
        initialize_value(dvm, func->local_variable[i].type->basic_type,
...
static void expand_stack(DVM_VirtualMachine *dvm, int need_stack_size) {
    ...
    if (rest <= need_stack_size) {
        ...
        dvm->stack.pointer_flags = MEM_realloc(
            dvm->stack.pointer_flags, dvm->stack.alloc_size * sizeof(DVM_Boolean));
    }
}

static void invoke_diksam_function(DVM_VirtualMachine *dvm, Function **caller_p,
                                   Function *callee, DVM_Byte **code_p,
                                   int *code_size_p, int *pc_p, int *sp_p,
                                   int *base_p, DVM_Executable **exe_p) {
    CallInfo *    callInfo;
    ...
    *exe_p   = callee->u.diksam_f.executable;
    ...
    expand_stack(
        dvm, CALL_INFO_ALIGN_SIZE + callee_p->local_variable_count + (*exe_p)->function[callee->u.diksam_f.index].need_stack_size);
...
static void return_function(DVM_VirtualMachine *dvm, Function **func_p,
    ...
    callInfo =
        (CallInfo *)&dvm->stack.stack[*sp_p - 1 - callee_p->local_variable_count - CALL_INFO_ALIGN_SIZE];
...
static DVM_Value execute(DVM_VirtualMachine *dvm, Function *func,
    ...
    for (pc = dvm->pc; pc < code_size;) {
        ...
        case DVM_PUSH_STRING:
            STO_WRITE(
                ...
                dvm_literal_to_dvm_string_i(
                    dvm,
                    exe->constant_pool[GET_2BYTE_INT(&code[pc + 1])].u.c_string));
            dvm->stack.stack_pointer++;
            pc += 3;
            break;
        ...
        case DVM_INVOKE: {
            ...
            if (dvm->function[func_idx].kind == NATIVE_FUNCTION) {
                ...
                invoke_diksam_function(dvm, &func, &dvm->function[func_idx], &code,
                                       &code_size, &pc, &dvm->stack.stack_pointer,
                                       &base, &exe);
            }
            break;
        }
